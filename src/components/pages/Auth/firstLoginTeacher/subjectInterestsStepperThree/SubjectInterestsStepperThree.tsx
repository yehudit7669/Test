...
const SubjectInterestsStepperThree = () => {
  ...
  const {firstLoginTeacherDetails, subjectsInterestChipDetails} = useAppSelector((state)=>state.firstLoginTeacher)
  ...
  const renderSubjectsInterestSelectableChips = () => {
    ...
    const handleGetSelectedChipsDetails = (multipleSelectableChipsArr:{id:string,label:string,selected:boolean}[]) => {
      ...
      dispatch(Actions.createAction(Actions.FIRST_LOGIN_TEACHER_UPDATE_SUBJECTS_INTEREST_CHIP_DETAILS,{multipleSelectableChipsArr}))
      /* Dispatching an action to update the selected state of the selectable chips */
      const dataToBeSent = {
        subjects:{
          ...firstLoginTeacherDetails.subjects,
          subjectNames: multipleSelectableChipsArr.filter((data)=>data.selected).map((data)=>data.id)
        }
      }
